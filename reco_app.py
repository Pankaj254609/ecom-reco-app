  ...
  with c_chart2:
    if not df_map.empty and "channelName" in df_map.columns:
      ...
      fig_pie = px.pie(
          ...
          names="Channel",
          values="Count",
          title="🌐 Channel Mapping Distribution",
          hole=0.4,
          color_discrete_sequence=px.colors.qualitative.Pastel,
      )
      st.plotly_chart(fig_pie, use_container_width=True)
    else:
      st.info("No Mapping Data available for chart visualization.")
  ...
  if settlement_file:
    ...
    if (
      ...
      with p_col1:
        ...
        total_payout = df_settle["Net_Payout"].sum()
        total_deduction = df_settle["Calculated_Deduction"].sum()

        df_breakdown = pd.DataFrame({
            "Category": ["Net Payout", "Deductions & Fees"],
            "Amount": [total_payout, total_deduction],
        })
        fig_settle = px.pie(
            df_breakdown,
            names="Category",
            values="Amount",
            title=f"Net Payout vs Deductions ({portal})",
      ...
      with p_col2:
        if "Order_ID" in df_settle.columns:
          fig_bar = px.bar(
              df_settle,
              x="Order_ID",
              y=["Net_Payout", "Calculated_Deduction"],
              title="Order-wise Payout vs Deductions",
              barmode="stack",
          )
...
# ==================== 🚚 RETURN RECONCILIATION ====================
  ...
  if ret_file:
    ...
    if "Claim_Status" in df_ret.columns:
      st.write("---")
      ...
      with r_col2:
        if "Return_Type" in df_ret.columns:
          df_type_summary = df_ret["Return_Type"].value_counts().reset_index()
          df_type_summary.columns = ["Return Type", "Count"]
          fig_type = px.pie(
              df_type_summary,
              names="Return Type",
              values="Count",
              title="Return Type Breakdown (RTO vs DTO)",
              hole=0.4,
          )
